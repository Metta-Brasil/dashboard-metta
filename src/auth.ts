 ...
 * ALLOWED_EMAILS existir, à lista dela (o gate mora no store).
 *
 ...
 *
 * ATENÇÃO: este arquivo é a raiz do bundle do middleware, que roda no
 * Edge. Nada de `@/lib/auth/users` em import estático — o store carrega
 * bcrypt e node:crypto, que o Edge não tem. Só import dinâmico, e só em
 * callback que nunca roda no middleware.
 */
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    async signIn({ account, profile }) {
      ...
      // Import dinâmico de `@/lib/auth/domain`, NÃO do store: domínio,
      // formato e allowlist ALLOWED_EMAILS moram lá, num módulo que não
      // carrega bcryptjs nem node:crypto. Importar de `users` puxaria o
      // store inteiro para o rastro do bundle do middleware (Edge), e um
      // módulo de Node inalcançável ali derruba o middleware na
      // inicialização — 500 em toda rota. Sem a env, o gate é só o
      // domínio, o de sempre.
