...
type PwReset = { code: string; attempts: number };
...
/**
 * Inicia o reset: valida que existe conta e-mail/senha, gera código de
 * 6 dígitos e grava `auth:pwreset:<email>` com TTL. Retorna o código
 * para o chamador enviar por e-mail. Conta Google (sem senha) → erro
 * orientando a entrar com Google.
 */
...
> {
  ...
  if (!u)
    return {
      ok: false,
      error:
        "Não encontramos uma conta com senha para esse e-mail. Se você entra com o Google, use “Continuar com Google”.",
    };
  const code = genCode();
  const saved = await redis([
    "SET",
    pwResetKey(e),
    JSON.stringify({ code, attempts: 0 } satisfies PwReset),
    "EX",
    String(PENDING_TTL),
  ]);
  if (saved === null)
    return { ok: false, error: "Falha ao iniciar. Tente de novo." };
  return { ok: true, email: e, name: u.name, code };
}

/** Reenvia o código de reset (novo código, zera tentativas, renova TTL). */
...
export async function confirmPasswordReset(
  ...
  if (String(code).trim() !== p.code) {
    await redis([
      "SET",
      pwResetKey(e),
      JSON.stringify({ ...p, attempts: p.attempts + 1 } satisfies PwReset),
      "EX",
      String(PENDING_TTL),
    ]);
    return { ok: false, error: "Código incorreto." };
